...
func (s *MerchantService) GetMerchants(ctx *gin.Context, data dto.GetMerchantsRequest) *dto.GetMerchantsResponse {
	db := s.db
	var (
	...
	if err != nil {
		errs.NewInternalError(ctx, "Failed to get merchants", err)
		return nil
	}
	defer rows.Close()
